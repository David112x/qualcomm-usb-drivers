...
INT MainRemovalTask(VOID)
{
   ...
   DisplayTime((PCHAR)"=== Start of Scanning ===", FALSE);
   ...
   DisplayTime((PCHAR)"=== End of Scanning ===", TRUE);
...
BOOL IsOSVersionVistaOrHigher(VOID)
{
   ...
   bResult = IsWindowsVistaOrGreater();
...
VOID ResetDriverConfig(VOID)
{
   ...
   (
	   (PTSTR)TEXT(SERVICE_KET_SER),
	   (PTSTR)TEXT("QCDriverSelectiveSuspendIdleTime"),
      0, TRUE
   );
   ResetServiceSettings
   (
	   (PTSTR)TEXT(SERVICE_KET_SER),
	   (PTSTR)TEXT("QCDriverWaitWakeEnabled"),
      0, TRUE
   );

   ResetServiceSettings
   (
	   (PTSTR)TEXT(SERVICE_KET_NET),
	   (PTSTR)TEXT("QCDriverSelectiveSuspendIdleTime"),
      0, TRUE
   );
   ResetServiceSettings
