...
#ifdef EVENT_TRACING
#include "QCDEV.tmh"      //  this is the file that will be auto generated
#endif
...
#define QCQDL_NAME L"\\QDLService"
static UNICODE_STRING QcQDLName;

BOOLEAN QCDEV_IsQDLRequest
(
   PDEVICE_EXTENSION pDevExt,
   PIRP              Irp
)
{
   ...
   RtlInitUnicodeString(&QcQDLName, QCQDL_NAME);

   if (fileObj->FileName.Length != 0)
   {
      isQDL = QCDEV_CompareFileName
              (
                 &QcQDLName,
                 &(fileObj->FileName)
              );
...
static UNICODE_STRING QcMBFileName;

BOOLEAN QCDEV_IsMBRequest
(
   PDEVICE_EXTENSION pDevExt,
   PIRP              Irp
)
{
   ...
   RtlInitUnicodeString(&QcMBFileName, FILTER_FILE_OBJECT_NAME);

   if (fileObj->FileName.Length != 0)
   {
      isMB = QCDEV_CompareFileName
              (
                 &QcMBFileName,
                 &(fileObj->FileName)
              );
