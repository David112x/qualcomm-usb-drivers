...
#ifdef EVENT_TRACING
#include "QCDSP.tmh"      //  this is the file that will be auto generated
#endif
...
NTSTATUS QCDSP_QueuedDispatch(IN PDEVICE_OBJECT CalledDO, IN PIRP Irp)
{
   ...
   {
      if (inDevState(DEVICE_STATE_DEVICE_REMOVED0))
      {
         QCSER_DispatchDebugOutput(Irp, irpStack, CalledDO, DeviceObject, irql);
         ...
         (
            (QCSER_DBG_MASK_CIRP | QCSER_DBG_MASK_CONTROL),
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> CIRP: 0x%p(DEL_PENDING) (dev in removal)\n", pDevExt->PortName, Irp)
         );
         return QcCompleteRequest(Irp, STATUS_DELETE_PENDING, 0);
      }
   ...
   if (irpStack->MajorFunction == IRP_MJ_SYSTEM_CONTROL)
   {
   ...
   // if (irpStack->MajorFunction == IRP_MJ_DEVICE_CONTROL)
   // if the IRP needs direct dispatch
   switch (irpStack->MajorFunction)
   {
      ...
      // the following is to satisfy logo test
      case IRP_MJ_PNP:
      {
...
)
{
   ...
   // Irp->IoStatus.Status = STATUS_PENDING;  // does not work with Verifier
...
VOID DispatchThread(PVOID pContext)
{
   ...
   pDispatchEvents[DSP_REQ_D0_EVENT_INDEX] = &(DeviceInfo[mgrId].DspReqD0Event);
...
dispatch_wait:
      ...
      switch (ntStatus)
      {
         ...
         case DSP_REQ_D0_EVENT_INDEX:
         {
            if (bDeviceRemoved == TRUE)
            {
               break;
            }
            KeClearEvent(&(DeviceInfo[mgrId].DspReqD0Event));
            QCSER_DbgPrint
            (
               QCSER_DBG_MASK_CONTROL,
               QCSER_DBG_LEVEL_DETAIL,
...
)
{
   ...
   switch (irpStack->MajorFunction)
   {
      case IRP_MJ_CREATE:
      {
         ...
         if (pDevExt->DeviceFunction == QCUSB_DEV_FUNC_QDL)
         {
            if (QCDEV_IsQDLRequest( pDevExt, Irp ) != TRUE)
            {
               ...
               (
                  QCSER_DBG_MASK_CONTROL,
                  QCSER_DBG_LEVEL_ERROR,
                  ("<%s> DSP: _CREATE, STATUS_UNSUCCESSFUL wrong file object\n", myPortName)
               );
               break;
            }
         }
      ...
      case IRP_MJ_CLEANUP:
      {
         ...
         {
            QCWT_SendAndWaitGPSC(pDevExt, QCWT_GPSC_STOP);
      ...
      case IRP_MJ_DEVICE_CONTROL:
      {
         ...
         switch (ioControlCode)
         {
            ...
            case IOCTL_QCUSB_SET_DBG_UMSK:
            {
               ...
               {
			   	  ULONG DebugMask = *((PULONG)ioBuffer);
#ifdef EVENT_TRACING				  
			   ...
			   else
			   {
                  ...
                  (
                     QCSER_DBG_MASK_CONTROL,
                     QCSER_DBG_LEVEL_ERROR,
                     ("<%s>: IOCTL_QCUSB_SET_DBG_UMSK - STATUS_INVALID_PARAMETER\n", pDevExt->PortName)
                  );
			   }
               break;
            }
            ...
            {
               Irp -> IoStatus.Status = SerialSendEncapusulatedCmd
                                        (
                                           DeviceObject,
                                           ioBuffer,
                                           Irp
         ...
         switch (irpStack->MinorFunction) 
         {
            ...
            case IRP_MN_START_DEVICE:  // PASSIVE_LEVEL
               ...
               if (NT_SUCCESS(ntStatus))
               {
                  ...
                  QCPNP_RetrieveServiceConfig(pDevExt);

                  if (pDevExt->bRemoteWakeupEnabled)
                  {
                      // QCPWR_RegisterWaitWakeIrp(pDevExt, 3);  // TODO-WW
                  }
            ...
            case IRP_MN_QUERY_STOP_DEVICE:  // PASSIVE_LEVEL
               if ((pDevExt->FDO == CalledDO) || (DeviceObject == CalledDO))
               {
                  StopInterruptService(pDevExt, TRUE, TRUE, 0);
            ...
            case IRP_MN_STOP_DEVICE:  // PASSIVE_LEVEL
               ...
               if ((pDevExt->FDO == CalledDO) || (DeviceObject == CalledDO))
               {
                  StopInterruptService(pDevExt, TRUE, TRUE, 1);
                  cancelAllIrps( DeviceObject );
                  ntStatus = QCPNP_StopDevice( DeviceObject );
                  if (NT_SUCCESS(ntStatus))
                  {
            ...
            case IRP_MN_QUERY_REMOVE_DEVICE:  // PASSIVE_LEVEL
               ...
               if ((pDevExt->FDO == NULL) || (pDevExt->FDO == CalledDO))
               {
                  ...
                  QCPWR_CancelIdleTimer(pDevExt, QCUSB_BUSY_CTRL, TRUE, 7);
               }
               else
               {
                  QCSER_DbgPrint
            ...
            case IRP_MN_SURPRISE_REMOVAL:  // PASSIVE_LEVEL
            {
               ...
               StopInterruptService(pDevExt, TRUE, TRUE, 2);
            ...
            case IRP_MN_REMOVE_DEVICE:  // PASSIVE_LEVEL
               if (pDevExt->RefCount <= 1)
               {
                  ...
                  if ((pDevExt->FDO == NULL) || (pDevExt->FDO == CalledDO))
                  {
                     ...
                     StopInterruptService(pDevExt, TRUE, TRUE, 3);
                     QcAcquireSpinLock(&pDevExt->ControlSpinLock, &levelOrHandle);
                     clearDevState(DEVICE_STATE_PRESENT_AND_STARTED);
                  ...
                  QCPNP_HandleRemoveDevice(DeviceObject, CalledDO, Irp, FALSE);
               ...
               else
               {
                  ...
                  (
                     QCSER_DBG_MASK_CONTROL,
                     QCSER_DBG_LEVEL_FORCE,
                     ("<%s> REMOVE: ASYNC - FDO 0x%p/rm0x%p PTDOflag 0x%x RefCnt %d\n",
                       myPortName, pDevExt->FDO, CalledDO, DeviceObject->Flags, pDevExt->RefCount)
                  );
                  if (pDevExt->FDO != CalledDO)
                  {
                     QCSER_RemoveFdoFromCollection(pDevExt, CalledDO);
                     QCPNP_HandleRemoveDevice(DeviceObject, CalledDO, Irp, TRUE); // remove only FDO
                  }
                  InterlockedDecrement(&(pDevExt->RefCount));
                  QcIoReleaseRemoveLock(pDevExt->pRemoveLock, Irp, 0);
               }
         ...
         else
         {
            if ((irpStack->MajorFunction == IRP_MJ_CLOSE) ||
                ((irpStack->MajorFunction == IRP_MJ_CREATE) &&
                 (ntStatus != STATUS_SUCCESS)) )
            {
               // remove the following conditon to prevent unbalanced RemoveLock when
               // port open/close happens between DEVICE_STATE_DEVICE_REMOVED0 event
               // and actual removal of the stack 
               // if (ntCloseStatus != STATUS_ALREADY_DISCONNECTED)
               {
...
NTSTATUS QCDSP_SendIrpToStack(IN PDEVICE_OBJECT PortDO, IN PIRP Irp, char *info)
{
   ...
   (
      ...
      ("<%s> %s: to stack-1 0x%p=>0x%p 0x%p[%d/%d:%d] RefC %d\n", pDevExt->PortName,
        info, Irp, currStackTopDO, pDevExt->FDO, Irp->CurrentLocation,
   ...
   if (irpStack->MajorFunction == IRP_MJ_CLOSE)
   {
      DbgPrint("<%s> CLOSE ToStack\n", pDevExt->PortName);
      bCloseReq = TRUE;
   }
   #endif  // TEST_CODE_CLOSE

   ntStatus = IoCallDriver(currStackTopDO, Irp);
   ...
   if (bCloseReq == TRUE)
   {
      DbgPrint("<%s> CLOSE ToStack rtn 0x%x\n", pDevExt->PortName, ntStatus);
   }
   #endif  // TEST_CODE_CLOSE
...
)
{
   ...
   if (pDevExt->bRemoteWakeupEnabled)
   {
       // QCPWR_RegisterWaitWakeIrp(pDevExt, 8);  // TODO-WW
   }
...
)
{
        ...
        (irpStack->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE)))
   {
      if (pDevExt->Sts.lRmlCount[0] > 1)
      {
         ...
         // to work around the UMDF issue, delay until no other pending IRPs 
         if (irpStack->MinorFunction == IRP_MN_QUERY_REMOVE_DEVICE)
         {
            // do we still have pending power IRP to process?
            QcAcquireSpinLock(&pDevExt->SingleIrpSpinLock, &levelOrHandle);
            if ((pDevExt->IdleNotificationIrp != NULL) ||
                (!IsListEmpty(&pDevExt->DevicePowerRequestQueue)))
            {
               QCSER_DbgPrint
               (
                  QCSER_DBG_MASK_CONTROL,
                  QCSER_DBG_LEVEL_ERROR,
                  ("<%s> _ToProcessIrp: hold for pending IRPs 0x%p\n", pDevExt->PortName,
            ...
            if (pDevExt->DeviceFunction == QCUSB_DEV_FUNC_GPS)
            {
               return (pDevExt->DevicePower == PowerDeviceD0);
            }
            return TRUE;
         }
         return FALSE;
...
VOID QCDSP_RequestD0(PDEVICE_EXTENSION pDevExt)
{
   ...
   else
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--RequestD0: RML failure\n", pDevExt->PortName)
      );
      return;
   }

   if (QCPWR_EnqueueDevicePowerRequest(pDevExt, PowerDeviceD0) == NULL)
   {
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--RequestD0: No dev req\n", pDevExt->PortName)
      );
      ...
      if (irp != NULL)
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> RequestD0: cancelling idle IRP 0x%p\n", pDevExt->PortName, irp)
         );
         IoCancelIrp(irp);
      }
   ...
   nts = PoRequestPowerIrp
         (
            pDevExt->PhysicalDeviceObject,
            IRP_MN_SET_POWER,
            powerState,
            (PREQUEST_POWER_COMPLETE)QCDSP_TopD0IrpCompletionRoutine,
            pDevExt,
            NULL
         );
   if (!NT_SUCCESS(nts))
...
)
{
   ...
   (
      QCSER_DBG_MASK_CONTROL,
      QCSER_DBG_LEVEL_DETAIL,
      ("<%s> TopD0IrpCompletionRoutine: 0x%x RML %d\n", pDevExt->PortName,
        IoStatus->Status, (pDevExt->Sts.lRmlCount[0]-1))
   );
   QCPWR_DequeueDevicePowerRequest(pDevExt);
   QcIoReleaseRemoveLock(pDevExt->pRemoveLock, NULL, 0);
}  // QCDSP_TopD0IrpCompletionRoutine
