...
#ifdef EVENT_TRACING
#include "QCINT.tmh"      //  this is the file that will be auto generated
#endif
...
// *************** END OF TEST CODE ******************
   ...
   ucHdlCnt++;
   if (InterlockedIncrement(&pDevExt->lIntThreadInCreation) > 1)
   {
      InterlockedDecrement(&pDevExt->lIntThreadInCreation);
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_READ,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> INT init in progress\n", pDevExt->PortName)
      );
      return STATUS_SUCCESS;
   }
   ...
   if (pDevExt->hInterruptThreadHandle == NULL)
   {
      pDevExt->pInterruptThread = NULL;
      InterlockedDecrement(&pDevExt->lIntThreadInCreation);
      QCSER_DbgPrint
...
VOID ReadInterruptPipe(IN PVOID pContext)
{
   ...
   BOOLEAN bCancelled = FALSE, bStopped = FALSE, bStopRegAccess = TRUE; // no reg access from 2057
   ...
   while(bKeepRunning)
   {
      if ((bIntReadActive == TRUE) || (pDevExt->bClosing == TRUE))
      {
...
wait_for_completion:
      ...
      switch (ntStatus)
      {
         case INT_COMPLETION_EVENT_INDEX:
         {
            ...
            if ((pDevExt->InterruptPipe == (UCHAR)-1) || (pDevExt->pInterruptBuffer == NULL))
            {
               break;
            }
         ...
         default:
         {
            ...
            (
               ...
               QCSER_DBG_LEVEL_CRITICAL,
               ("<%s> STA: [%s] svc %u/%u Rstp %u/%u Ron %u/%u Won %u Srm %u RM %u ST 0x%x Susp %u Lk %ld\n",
