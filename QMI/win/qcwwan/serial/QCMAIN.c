...
#include "QCMAIN.tmh"      //  this is the file that will be auto generated
#endif
...
)
{
   ...
   if ( !NT_SUCCESS( ntStatus ) )
   {
      ...
      (
         QCSER_DBG_MASK_CONTROL,
         QCSER_DBG_LEVEL_ERROR,
         ("\n<%s> MgrThread Failure", gDeviceName)
      );
...
NTSTATUS QCSER_CleanupDeviceExtensionBuffers(IN PDEVICE_OBJECT DeviceObject)
{
   ...
   {
      ExFreePool(pDevExt->WriteAggBuffer.Buffer);
      pDevExt->WriteAggBuffer.Buffer = NULL;
   }
   ...
   QCPWR_PurgeDevicePowerQueue(pDevExt);
...
)
{
   ...
   if (DeviceObject == NULL)
   {
      ...
      sprintf(msg, "<%s 0X%p::0x%p:IRQL-%d:0x%p(0x%08X)> ", gDeviceName, CalledDO, DeviceObject, irql, Irp, Irp->IoStatus.Status);
   }
   else
   {
      pDevExt = DeviceObject->DeviceExtension;
      ...
      sprintf(msg, "<%s 0X%p::0x%p:IRQL-%d:0x%p(0x%08X)> ", pDevExt->PortName, CalledDO, DeviceObject, irql, Irp, Irp->IoStatus.Status);
   }
   msg += strlen(msg);
