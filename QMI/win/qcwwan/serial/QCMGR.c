...
#ifdef EVENT_TRACING
#include "QCMGR.tmh"      //  this is the file that will be auto generated
#endif
...
NTSTATUS QCMGR_StartManagerThread(VOID)
{
   ...
   for (element = 0; element < MGR_MAX_NUM_DEV; element++)
   {
      ...
      KeInitializeEvent
      (
         &(DeviceInfo[element].DspReqD0Event),
         NotificationEvent,
         FALSE
      );
   ...
   if (!NT_SUCCESS(ntStatus))
   {
      ...
      (
         QCSER_DBG_MASK_CONTROL,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> MGR: ObReferenceObjectByHandle failed 0x%x\n", gDeviceName, ntStatus)
      );
      pMgrThread = NULL;
   }
   else
