...
#ifdef EVENT_TRACING
#include "QCMRD.tmh"      //  this is the file that will be auto generated
#endif
...
wait_for_completion2:
      ...
      if ((pDevExt->pL2ReadBuffer[pDevExt->L2FillIdx].State == L2BUF_STATE_COMPLETED) &&
          (pDevExt->pL2ReadBuffer[pDevExt->L2FillIdx].bInCompletionQueue == FALSE) &&
      ...
      switch(ntStatus)
      {
         ...
         case L1_READ_COMPLETION_EVENT_INDEX:
         {
            ...
            if ((pDevExt->pL2ReadBuffer[pDevExt->L2FillIdx].bReturnToUser == FALSE) ||
				 ((!NT_SUCCESS(pDevExt->pL2ReadBuffer[pDevExt->L2FillIdx].Status)) && (pCurrIOBlock == NULL)) ||
              ...
              inDevState(DEVICE_STATE_PRESENT_AND_STARTED))
      {
         ...
         if (pDevExt->bL2Stopped == FALSE)
         {
            ...
            if (lBytesInQueue >= pDevExt->lReadBuffer80pct)
            {
               ...
               //    try to process any queued IOCTL_SERIAL_WAIT_ON_MASK when we get here.
            ...
            if (ntStatus != STATUS_PENDING)
            {
			   ...
			   /* Do not set the state, since it gets set in completion routine */
      ...
      switch (ntStatus)
      {
         ...
         default:
         {
            ...
            if (!IsListEmpty(&pDevExt->L2CompletionQueue))
            {
			   ...
			   pActiveL2Buf->bInCompletionQueue = FALSE;
            }
            ...
            (
               QCSER_DBG_MASK_READ,
               QCSER_DBG_LEVEL_DETAIL,
               ("<%s> L2 completion [%u]=wait rtn[%u] %dB\n", pDevExt->PortName,
                 l2BufIdx, (ntStatus-L2_READ_EVENT_COUNT),
            ...
            else
            {
               ...
               (
                  QCSER_DBG_MASK_READ,
                  QCSER_DBG_LEVEL_CRITICAL,
                  ("<%s> L2 RX failed[%u] - 0x%x/0x%x ToUser %d\n", pDevExt->PortName, l2BufIdx,
                    pUrb->UrbHeader.Status, pIrp->IoStatus.Status, pDevExt->bL1PropagateCancellation)
               );
            }
            #endif  // ENABLE_LOGGING
