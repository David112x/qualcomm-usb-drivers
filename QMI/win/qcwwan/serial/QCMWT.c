...
#ifdef EVENT_TRACING
#include "QCMWT.tmh"      //  this is the file that will be auto generated
#endif
...
VOID QCMWT_WriteThread(PVOID pContext)
{
   ...
   LARGE_INTEGER aggregateTime;
   BOOLEAN       bAggTimerArmed = FALSE;
   #ifdef QCSER_TARGET_XP
   KLOCK_QUEUE_HANDLE levelOrHandle;
   #else
   KIRQL levelOrHandle;
   ...
   while(TRUE)
   {
      ...
      while (TRUE)
      {
         ...
         if (((pDevExt->pWriteHead) || (bSendZeroLength == TRUE) || ((pDevExt->AggregationEnabled == 1) && (pDevExt->bAggDispatch == TRUE))) &&
             ...
             (inDevState(DEVICE_STATE_PRESENT_AND_STARTED)))
         {
            ...
            if ((pDevExt->AggregationEnabled == 1) && (pDevExt->WriteAggBuffer.State == AGG_STATE_BUSY))
            {
               QCSER_DbgPrint
               (
                  QCSER_DBG_MASK_WRITE,
                  QCSER_DBG_LEVEL_DETAIL,
                  ("<%s> _MWT: AGG_STATE_BUSY, wait 0x%p", pDevExt->PortName, pDevExt->pWriteHead)
               );
               QcReleaseSpinLock(&pDevExt->WriteSpinLock, levelOrHandle);
               goto wait_for_completion;  // wait for the kick event
            }
            ...
            {
	            // If fully aggregated, dispatch right away
                // To be reviewed : Murali
	            if (((pDevExt->WriteAggBuffer.CurrentCount >= pDevExt->WriteAggBuffer.MaxSize) ||
	                 (pDevExt->WriteAggBuffer.OkToSend == TRUE)) &&
	                (pDevExt->bAggDispatch == FALSE))
	            {
	               QCSER_DbgPrint
	               (
	                  QCSER_DBG_MASK_WRITE,
	                  QCSER_DBG_LEVEL_DETAIL,
	                  ("<%s> _MWT: fully agg, send", pDevExt->PortName)
	               );
	               pDevExt->bAggDispatch = TRUE;
	            }
            }
			...
			if (pDevExt->AggregationEnabled == 1)
			{
	            if ((pDevExt->bAggDispatch == TRUE) && (pDevExt->pWriteHead == NULL))
	            {
	               if (pDevExt->WriteAggBuffer.CurrentCount == 0)
	               {
	                  QCSER_DbgPrint
	                  (
	                     QCSER_DBG_MASK_WRITE,
	                     QCSER_DBG_LEVEL_DETAIL,
	                     ("<%s> _MWT: agg none, pause", pDevExt->PortName)
	                  );
	                  pDevExt->bAggDispatch = FALSE;
	                  // revert
	               ...
	               else
	               {
	                  ...
	                  (
	                     QCSER_DBG_MASK_WRITE,
	                     QCSER_DBG_LEVEL_DETAIL,
	                     ("<%s> _MWT: agg true, send", pDevExt->PortName)
	                  );
	            ...
	            if ((bSendZeroLength == FALSE) && (pDevExt->pWriteHead != NULL) &&
	                (pDevExt->bAggDispatch == FALSE))
	            {
					// Dequeue write IRP
            ...
            else if ((bCancelled == TRUE) || pCurrIOBlock->bPurged == TRUE)
            {
 		    ...
 		    else if ((pDevExt->AggregationEnabled == 1) && (pDevExt->bAggDispatch == FALSE)) // condition must be TRUE, unnecessary
 		    {
 			  if ((pDevExt->WriteAggBuffer.CurrentCount < pDevExt->WriteAggBuffer.MaxSize) &&
 				  (pDevExt->WriteAggBuffer.OkToSend == FALSE))
 			  {
 				 // continue to fill aggregate buffer
 				 NTSTATUS nts;

				 if (pDevExt->AggregationEnabled == 1 && pDevExt->TimerResolutionSet == 0)
			     {
				    ULONG returnVal = 0;
					
					// Set the timer resolution
					returnVal = ExSetTimerResolution(pDevExt->ResolutionTime*10000, TRUE);
					QCSER_DbgPrint
					(
					   QCSER_DBG_MASK_CONTROL,
					   QCSER_DBG_LEVEL_DETAIL,
					   ("<%s> MPth: Modem port opened set ExSetTimerResolution returned %d ResolutionTime : %d\n", pDevExt->PortName, 
					   returnVal, pDevExt->ResolutionTime)
					);
					pDevExt->TimerResolutionSet = 1;
				 }				 
 		   
 				 if (bAggTimerArmed == FALSE)
 				 {
 					LONG tmpVal = pDevExt->AggregationPeriod * 10;
 					bAggTimerArmed = TRUE;
 					aggregateTime.QuadPart = -(tmpVal);
 					QCSER_DbgPrint
 					(
 					   QCSER_DBG_MASK_WRITE,
 					   QCSER_DBG_LEVEL_DETAIL,
 					   ("<%s> _MWT: start agg timer: %ldms", pDevExt->PortName, (tmpVal/10000))
 					);
 					KeSetTimer(&pDevExt->AggregateTimer, aggregateTime, &pDevExt->AggregateDpc);
 				 }
 		   
                 // if it
 				 nts = QCMWT_AggregatePcaket(pDevExt, pCurrIOBlock);
 				 if (nts == STATUS_SUCCESS)
 				 {
 					// complete the IRP
 					pCurrIOBlock->ntStatus = STATUS_SUCCESS;
 					pCurrIOBlock->pCompletionRoutine(pCurrIOBlock, TRUE, 20);
 					QcExFreeWriteIOB(pCurrIOBlock, TRUE);
 			  ...
 			  (
 				 QCSER_DBG_MASK_WRITE,
 				 QCSER_DBG_LEVEL_DETAIL,
 				 ("<%s> _MWT: Re-Q IOB 0x%p\n", pDevExt->PortName, pDevExt->pWriteHead)
 			  );
 				  ...
 				  (bAggTimerArmed == TRUE))
 			  {
 				 ...
 				 // fully aggregated, cancel timer
 				 inQueue = KeCancelTimer(&pDevExt->AggregateTimer);
 				 if (inQueue == FALSE)
 				 {
 					// DPC is running
 					QCSER_DbgPrint
 				 ...
 				 else
 				 {
 					...
 					(
 					   QCSER_DBG_MASK_WRITE,
 					   QCSER_DBG_LEVEL_DETAIL,
 					   ("<%s> _MWT: fully agg'ed, set TRUE\n", pDevExt->PortName)
 					);
 					pDevExt->bAggDispatch = TRUE;
 					bAggTimerArmed = FALSE;
 				 }
 			  }
 			  QCSER_DbgPrint
 			  (
 				 QCSER_DBG_MASK_WRITE,
 				 QCSER_DBG_LEVEL_DETAIL,
 				 ("<%s> _MWT: go round again, %u/%u\n", pDevExt->PortName, pDevExt->bAggDispatch, bSendZeroLength)
 			  );
 			  continue;
 		   }  // if (pDevExt->bAggDispatch == FALSE)
   ...
   MWT_Irp_Setup:
            ...
            if (pDevExt->AggregationEnabled == 1)
            {
				...
				(
				   QCSER_DBG_MASK_WRITE,
				   QCSER_DBG_LEVEL_DETAIL,
				   ("<%s> _MWT: MWT_Irp_Setup %u/%uB HD 0x%p", pDevExt->PortName,
					 bSendZeroLength, pDevExt->WriteAggBuffer.CurrentCount, pDevExt->pWriteHead)
				);
            
	            // setup irp for i/o
				if ((bSendZeroLength == FALSE) && (pDevExt->WriteAggBuffer.CurrentCount != 0))
				{
	               ...
	               {
					   pActiveBuffer = (PCHAR)(pDevExt->WriteAggBuffer.Buffer);
					   ulTransferBytes = pDevExt->WriteAggBuffer.CurrentCount;
	               }
	            }
	            else
	            {
...
#error code not present
            ...
            // Need to review for aggrigation mode
            ...
            if (ulTransferBytes > 0)
            {
               ...
               (
                  ...
                  (pWtBuf->IoBlock != NULL ? pWtBuf->IoBlock->pCallingIrp: 0), pIrp, pWtBuf->Index, ulTransferBytes, irpSentRec->TotalLength)
               );
            }
            else
            {
	...
	#error code not present
           ...
           if (pDevExt->AggregationEnabled == 1)
           {
		      pDevExt->WriteAggBuffer.State = AGG_STATE_BUSY;
		      pDevExt->WriteAggBuffer.OkToSend = FALSE;
           }
...
process_event:
      ...
      switch(ntStatus)
      {
         ...
         case WRITE_AGGREGATE_EVENT_INDEX:
         {
            QCSER_DbgPrint
            (
               QCSER_DBG_MASK_WRITE,
               QCSER_DBG_LEVEL_DETAIL,
               ("<%s> W WRITE_AGGREGATE: stopped %d", pDevExt->PortName,
                 pDevExt->bWriteStopped)
            );
            KeClearEvent(&pDevExt->WriteAggregateEvent);
            bAggTimerArmed = FALSE;
            if (pDevExt->bWriteStopped == FALSE)
            {
               pDevExt->bAggDispatch = TRUE;
            }
            break;
         ...
         default:
         {
            ...
            if (pDevExt->AggregationEnabled == 1)
            {
               pDevExt->WriteAggBuffer.State = AGG_STATE_IDLE;
               pDevExt->WriteAggBuffer.CurrentCount = 0;
               pDevExt->bAggDispatch = FALSE;
            }
            ...
            if (((pDevExt->AggregationEnabled == 1) && (pWtBuf->Length == 0)) ||
				((pDevExt->AggregationEnabled == 0) && (pCurrIOBlock == NULL)))
            {
               // need to handle here
            ...
            if (ntStatus == STATUS_SUCCESS)
            {
			   ...
			   if (((pDevExt->AggregationEnabled == 1) && (ulTransferBytes < pWtBuf->Length)) ||
				   ((pDevExt->AggregationEnabled == 0) && (pCurrIOBlock->ulActiveBytes > 0)))
               {
                  QCSER_DbgPrint
                  (
                     QCSER_DBG_MASK_WRITE,
...
exit_WriteThread:
   ...
   KeCancelTimer(&pDevExt->AggregateTimer);
...
NTSTATUS QCMWT_InitializeMultiWriteElements(PDEVICE_EXTENSION pDevExt)
{
   ...
   if (pDevExt->AggregationEnabled == 1)
   {
	   if (pDevExt->WriteAggBuffer.Buffer == NULL)
	   {
	      pDevExt->WriteAggBuffer.Buffer = ExAllocatePool
	                                       (
	                                          NonPagedPool,
	   ...
	   {
	      DbgPrint("<%s> WARNING: ==== MWT AGG buf not NULL!!!\n", pDevExt->PortName);
	      goto MWT_InitExit;
	   }
   }
...
{
   PIRP pIrp = Iob->pCallingIrp;
   PCHAR destBuf = QC_DEST_AGG_BUF;
   PCHAR srcBuf;
   ULONG availableBytes = QC_AVAILABLE_AGG_BYTES;

   Iob->ulActiveBytes = Iob->ulBTDBytes - pIrp->IoStatus.Information;
   srcBuf = (PCHAR)Iob->pBufferToDevice + pIrp->IoStatus.Information;

   QCSER_DbgPrint
   (
      QCSER_DBG_MASK_WRITE,
      QCSER_DBG_LEVEL_DETAIL,
      ("<%s> -->_AggregatePcaket 0x%p(%uB) %u>>", pDevExt->PortName,
        pIrp, Iob->ulActiveBytes, pDevExt->WriteAggBuffer.CurrentCount)
   );

   if (availableBytes >= Iob->ulActiveBytes)
   {
      // aggregate bytes
      RtlCopyMemory(destBuf, srcBuf, Iob->ulActiveBytes);
      pDevExt->WriteAggBuffer.CurrentCount += Iob->ulActiveBytes;
      pIrp->IoStatus.Information += Iob->ulActiveBytes;
      ...
      (
         QCSER_DBG_MASK_WRITE,
         QCSER_DBG_LEVEL_DETAIL,
         ("<%s> <--_AggregatePcaket %uB/%uB/%uB", pDevExt->PortName,
          Iob->ulActiveBytes, pIrp->IoStatus.Information,
          pDevExt->WriteAggBuffer.CurrentCount)
      );
   ...
   if (availableBytes == 0)
   {
      ...
      (
         QCSER_DBG_MASK_WRITE,
         QCSER_DBG_LEVEL_DETAIL,
         ("<%s> <--_AggregatePcaket: buf full 0x%p", pDevExt->PortName, pIrp)
      );
      return STATUS_NO_MEMORY;
   }
   ...
   RtlCopyMemory(destBuf, srcBuf, availableBytes);
   pDevExt->WriteAggBuffer.CurrentCount += availableBytes;
   pIrp->IoStatus.Information += availableBytes;
   QCSER_DbgPrint
   (
      QCSER_DBG_MASK_WRITE,
      QCSER_DBG_LEVEL_DETAIL,
      ("<%s> <--_AggregatePcaket: part 0x%p(%uB/%uB/%uB)", pDevExt->PortName, pIrp,
        availableBytes, pIrp->IoStatus.Information, pDevExt->WriteAggBuffer.CurrentCount)
   );
   return STATUS_NO_MEMORY;
   ******************/
}  // QCMWT_AggregatePcaket
...
)
{
   ...
   (
      QCSER_DBG_MASK_WRITE,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> _WriteAggregateDpc", pDevExt->PortName)
   );
   KeSetEvent
   (
      &pDevExt->WriteAggregateEvent,
      IO_NO_INCREMENT,
      FALSE
   );

}  // QCPWR_WriteAggregateDpc
