...
#ifdef EVENT_TRACING
#include "QCPTDO.tmh"      //  this is the file that will be auto generated
#endif
...
)
{
   ...
   InterlockedIncrement(&(pDevExt->RefCount));
   ...
   // allocate buffer for interrupt pipe
   pDevExt->pInterruptBuffer = ExAllocatePool
                               (
                                  NonPagedPool, QCSER_RECEIVE_BUFFER_SIZE
                               );
   ...
   (
      ...
      ("<%s> SpinLocks: C:(0x%x, 0x%p) R:(0x%x, 0x%p) W:(0x%x, 0x%p) S:(0x%x, 0x%p) P:(0x%x, 0x%p)\n", myPortName,
   ...
   pDevExt->pWriteEvents[WRITE_AGGREGATE_EVENT_INDEX] = &pDevExt->WriteAggregateEvent;
   KeInitializeEvent(&pDevExt->WriteAggregateEvent, NotificationEvent, FALSE);
   KeInitializeTimer(&pDevExt->AggregateTimer);
   KeInitializeDpc(&pDevExt->AggregateDpc, QCPWR_WriteAggregateDpc, pDevExt);
   // Set the DPC priority here
   ...
   pDevExt->lIntThreadInCreation = 0;
   ...
   KeInitializeEvent
   (
      &pDevExt->GPSStartEvent,
      NotificationEvent,
      FALSE
   );
   KeInitializeEvent
   (
      &pDevExt->GPSStopEvent,
      NotificationEvent,
      FALSE
   );
...
)
{
   ...
   if (myPortDO != NULL)
   {
      ...
      InterlockedIncrement(&(portDoExt->RefCount));
      
      portDoExt->bStackOpen = FALSE;
      ...
      while (TRUE)
      {
         if (InterlockedIncrement(&portDoExt->ActivityInProgress) > 1)
         {
            InterlockedDecrement(&portDoExt->ActivityInProgress);
   
            QCSER_DbgPrintG
            (
               QCSER_DBG_MASK_CONTROL,
               QCSER_DBG_LEVEL_INFO,
               ("<%s> [SYNC]: PTDO to wait for dispatch completion\n", myPortName)
            );

            QCSER_Wait(portDoExt, -(2 * 1000 * 1000L));  // 0.2 second
         }
         else
         {
            break;
