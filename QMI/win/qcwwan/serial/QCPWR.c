...
#ifdef EVENT_TRACING
#include "QCPWR.tmh"      //  this is the file that will be auto generated
#endif
...
)
{
   ...
   if (pDevExt->bRemoteWakeupEnabled == TRUE)
   {
      ...
      if (PowerState > pDevExt->SystemWakeupLevel)
      {
      ...
      {
         // Issue Wait-Wake IRP
         // QCPWR_RegisterWaitWakeIrp(pDevExt, 5); // TODO-WW
      }
   }
...
)
{
   ...
   if (QCPWR_EnqueueDevicePowerRequest(pDevExt, powState.DeviceState) == NULL)
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> PIRP (Csys 0) 0x%p (DPWR) No dev req [PrepareToPowerDown %d]\n",
           pDevExt->PortName, Irp, pDevExt->PrepareToPowerDown)
      );
      IoCompleteRequest(Irp, IO_NO_INCREMENT);
      QcIoReleaseRemoveLock(pDevExt->pRemoveLock, Irp, 0);
   ...
   if ((!NT_SUCCESS(ntStatus)) || (powerContext->Irp == NULL))
   {
      if (!NT_SUCCESS(ntStatus))
      {
         QCPWR_DequeueDevicePowerRequest(pDevExt);
      }
      QcIoReleaseRemoveLock(pDevExt->pRemoveLock, powerContext, 0);
      PoStartNextPowerIrp(Irp);
...
)
{
   ...
   if (pDevExt->bRemoteWakeupEnabled == TRUE)
   {
      ...
      else if (PowerState > PowerSystemWorking)
      {
         // Issue Wait-Wake IRP
         QCPWR_RegisterWaitWakeIrp(pDevExt, 4);  // TODO-WW
      }
   }
...
)
{
   ...
   if((pDevExt->DevicePower == PowerDeviceD0) && (NewDevicePower > pDevExt->DevicePower))
   {
      ...
      if (inDevState(DEVICE_STATE_PRESENT_AND_STARTED))
      {
         ...
         QCSER_StopDataThreads(pDevExt, FALSE, TRUE);
...
)
{
   ...
   (
      QCSER_DBG_MASK_PIRP,
      QCSER_DBG_LEVEL_DETAIL,
      ("<%s> -->CancelIdleTimer(%u): BusyMask 0x%x RM[0] %d\n", pDevExt->PortName, Cookie, BusyMask,
...
)
{
   ...
   if ((pDevExt->WdmVersion < WinXpOrHigher)                   ||
       ((pDevExt->SelectiveSuspendInMiliSeconds == TRUE) && (pDevExt->SelectiveSuspendIdleTime < QCUSB_SS_MILI_IDLE_MIN)) ||   	
   ...
   if (inDevState(DEVICE_STATE_DEVICE_QUERY_REMOVE))
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--SetIdleTimer(%u): dev in query-removal\n", pDevExt->PortName, Cookie)
      );
      QcReleaseSpinLockWithLevel(&pDevExt->SingleIrpSpinLock, levelOrHandle, irql);
      return;
   }
   ...
   if (pDevExt->IoBusyMask == 0)
   {
      ...
      {
		  if (pDevExt->SelectiveSuspendInMiliSeconds == TRUE)
		  {
...
)
{
   ...
   if (KeReadStateTimer(&pDevExt->IdleTimer) == TRUE)  // not been reset
   {
      ...
      if (pDevExt->SystemPower <= pDevExt->SystemWakeupLevel)
      {
...
NTSTATUS QCPWR_RegisterIdleNotification(PDEVICE_EXTENSION pDevExt)
{
   ...
   if (inDevState(DEVICE_STATE_DEVICE_QUERY_REMOVE))
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> RegisterIdleNotification: in query-removal\n", pDevExt->PortName)
      );
      ntStatus = STATUS_POWER_STATE_INVALID;
      goto RegisterIdleNotification_Exit;
   }
   ...
   if (idleCallbackInfo != NULL)
   {
      ...
      else
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> RegisterIdleNotification: Submit idle req 0x%p at D%u\n",
              pDevExt->PortName, irp, pDevExt->DevicePower-1)
         );
...
VOID QCPWR_IdleNotificationCallbackEpisode(PDEVICE_EXTENSION pDevExt)
{
   ...
   if (inDevState(DEVICE_STATE_DEVICE_QUERY_REMOVE))
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--IdleNotificationCallbackEpisode: dev in query-removal\n", pDevExt->PortName)
      );
      return;
   }
   ...
   else
   {
      ...
      if (QCPWR_EnqueueDevicePowerRequest(pDevExt, powerState.DeviceState) == NULL)
      {
         QCSER_DbgPrint
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> IdleNotificationCallbackEpisode(DPWR): No dev req\n", pDevExt->PortName)
         );
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> IdleNotificationCallbackEpisode: PWR req for D%u sent, status 0x%x\n",
...
)
{
   ...
   if ((ntStatus != STATUS_POWER_STATE_INVALID) ||  // condition for MCCI
       ...
       (pDevExt->WaitWakeEnabled == FALSE))
   {
      if (pDevExt->PrepareToPowerDown == TRUE)
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> IdleNotificationIrpCompletionEpisode: err 0x%x, powering down-no act\n", pDevExt->PortName, ntStatus)
         );
         goto ExitPoint;
      }
      ...
      if (pDevExt->DevicePower == PowerDeviceD0)
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> IdleNotificationIrpCompletionEpisode: in D0, no act\n", pDevExt->PortName)
         );
         goto ExitPoint;
      }
      ...
      if (QCPWR_EnqueueDevicePowerRequest(pDevExt, powerState.DeviceState) == NULL)
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> IdleNotificationIrpCompletionEpisode(DPWR): No dev req\n", pDevExt->PortName)
         );
...
ExitPoint:
   ...
   (
      QCSER_DBG_MASK_PIRP,
      QCSER_DBG_LEVEL_DETAIL,
      ("<%s> <--IdleNotificationIrpCompletionEpisode\n", pDevExt->PortName)
   );
...
VOID QCPWR_CancelIdleNotificationIrp(PDEVICE_EXTENSION pDevExt, UCHAR Cookie)
{
   ...
   (
      QCSER_DBG_MASK_PIRP,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> -->CancelIdleNotificationIrp(%u): 0x%p RM[0] %d\n", pDevExt->PortName, Cookie,
   ...
   if (pDevExt->IdleNotificationIrp != NULL)
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_DETAIL,
         ("<%s> CancelIdleNotificationIrp: req D0 (MgrId %d)\n", pDevExt->PortName, pDevExt->MgrId)
      );
      KeSetEvent
      (
         &(DeviceInfo[pDevExt->MgrId].DspReqD0Event),
         IO_NO_INCREMENT,
         FALSE
      );
   }
...
VOID QCPWR_RegisterWaitWakeIrp(PDEVICE_EXTENSION  pDevExt, UCHAR Cookie)
{
   ...
   if (inDevState(DEVICE_STATE_DEVICE_QUERY_REMOVE))
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--RegisterWaitWake(%u): dev in query-removal\n", pDevExt->PortName, Cookie)
      );
      return;
   }
...
)
{
   ...
   if (QCPWR_EnqueueDevicePowerRequest(pDevExt, powerState.DeviceState) == NULL)
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> TopWaitWakeCompletion(DPWR): No dev req\n", pDevExt->PortName)
      );
      QcIoReleaseRemoveLock(pDevExt->pRemoveLock, pDevExt, 0);
      return;
   }
...
)
{
   ...
   (
      QCSER_DBG_MASK_PIRP,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> <--CheckToWakeup(%u): IRP 0x%p Susp %u hold %u RM[0] %d\n", pDevExt->PortName,
        Cookie, Irp, pDevExt->PowerSuspended, holdRequest, pDevExt->Sts.lRmlCount[0])
   );
...
VOID QCPWR_RegisterWmiPowerGuids(PDEVICE_EXTENSION pDevExt)
{
   ...
   if ((!inDevState(DEVICE_STATE_PRESENT_AND_INIT)) || (pDevExt->FDO == NULL))
   {
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_TRACE,
         ("<%s> <--RegisterWmiPowerGuids: dev not init\n", pDevExt->PortName)
      );
      return;
   }
...
VOID QCPWR_DeregisterWmiPowerGuids(PDEVICE_EXTENSION pDevExt)
{
   ...
   (
      QCSER_DBG_MASK_PIRP,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> <--DeregisterWmiPowerGuids ST 0x%x\n", pDevExt->PortName, ntStatus)
   );
...
)
{
   ...
   if ((IsEnabled == TRUE) && (pDevExt->WaitWakeEnabled == FALSE))
   {
      ...
      // QCPWR_RegisterWaitWakeIrp(pDevExt, 6); // WW is engaged right before SS, not here
...
{
	if ((((pDevExt->SelectiveSuspendInMiliSeconds == TRUE) && (pDevExt->SelectiveSuspendIdleTime >= QCUSB_SS_MILI_IDLE_MIN)) ||
		(pDevExt->SelectiveSuspendIdleTime >= QCUSB_SS_IDLE_MIN)) &&
       (pDevExt->PowerManagementEnabled == TRUE))
   {
...
PQCUSB_DEVICE_POWER_REQ QCPWR_EnqueueDevicePowerRequest
(
   PDEVICE_EXTENSION  pDevExt,
   DEVICE_POWER_STATE PowerState
)
{
   PQCUSB_DEVICE_POWER_REQ req;
   #ifdef QCSER_TARGET_XP
   KLOCK_QUEUE_HANDLE levelOrHandle;
   #else
   KIRQL levelOrHandle;
   ...
   (
      QCSER_DBG_MASK_PIRP,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> -->EnqueueDevicePowerRequest(DPWR): %u\n", pDevExt->PortName, PowerState)
   );

   if (pDevExt->DevicePower == PowerState)
   {
      BOOLEAN toEnqueue = FALSE;

      QcAcquireSpinLock(&pDevExt->SingleIrpSpinLock, &levelOrHandle);
      if (!IsListEmpty(&pDevExt->DevicePowerRequestQueue))
      {
         // there must be different dev pwr request pending
         toEnqueue = TRUE;
      }
      QcReleaseSpinLock(&pDevExt->SingleIrpSpinLock, levelOrHandle);
      if (toEnqueue == FALSE)
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> <--EnqueueDevicePowerRequest(DPWR): already in D%u\n", pDevExt->PortName, PowerState-1)
         );
         return NULL;
      }
      else
      {
         ...
         (
            QCSER_DBG_MASK_PIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> EnqueueDevicePowerRequest(DPWR): in D%u, req again\n", pDevExt->PortName, PowerState-1)
         );
      }
   }

   if (TRUE == QCPWR_IsSameDevicePowerRequested(pDevExt, PowerState))
   {
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--EnqueueDevicePowerRequest(DPWR): DUP D%u\n", pDevExt->PortName, PowerState-1)
      );
      return NULL;
   }

   req = ExAllocatePool(NonPagedPool, sizeof(QCUSB_DEVICE_POWER_REQ));
   if (req == NULL)
   {
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> <--EnqueueDevicePowerRequest(DPWR): NO_MEM\n", pDevExt->PortName)
      );
      return NULL;
   }

   req->DevicePower = PowerState;

   QcAcquireSpinLock(&pDevExt->SingleIrpSpinLock, &levelOrHandle);
   InsertTailList(&pDevExt->DevicePowerRequestQueue, &req->List);
   QcReleaseSpinLock(&pDevExt->SingleIrpSpinLock, levelOrHandle);
...
}  // QCPWR_EnqueueDevicePowerRequest

VOID QCPWR_DequeueDevicePowerRequest(PDEVICE_EXTENSION pDevExt)
{
   PLIST_ENTRY             headOfList;
   PQCUSB_DEVICE_POWER_REQ req = NULL;
   #ifdef QCSER_TARGET_XP
   KLOCK_QUEUE_HANDLE levelOrHandle;
   #else
   KIRQL levelOrHandle;
   ...
   if (!IsListEmpty(&pDevExt->DevicePowerRequestQueue))
   {
      headOfList = RemoveHeadList(&pDevExt->DevicePowerRequestQueue);
      req = CONTAINING_RECORD(headOfList, QCUSB_DEVICE_POWER_REQ, List);
   } 
   QcReleaseSpinLock(&pDevExt->SingleIrpSpinLock, levelOrHandle);
   ...
   else
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> DequeueDevicePowerRequest(DPWR): error (no req)\n", pDevExt->PortName)
      );
   }

}  // QCPWR_DequeueDevicePowerRequest

BOOLEAN QCPWR_IsSameDevicePowerRequested
(
   PDEVICE_EXTENSION  pDevExt,
   DEVICE_POWER_STATE PowerState
)
{
   PLIST_ENTRY             tailOfList, peekEntry;
   PQCUSB_DEVICE_POWER_REQ req = NULL;
   #ifdef QCSER_TARGET_XP
   KLOCK_QUEUE_HANDLE levelOrHandle;
   #else
   ...
   if (!IsListEmpty(&pDevExt->DevicePowerRequestQueue))
   {
      tailOfList = &pDevExt->DevicePowerRequestQueue;
      peekEntry = tailOfList->Blink;
      req = CONTAINING_RECORD(peekEntry, QCUSB_DEVICE_POWER_REQ, List);
      if (req->DevicePower == PowerState)
      {
         QcReleaseSpinLock(&pDevExt->SingleIrpSpinLock, levelOrHandle);
         return TRUE;
      }
   ...
   return FALSE;
}  // QCPWR_IsSameDevicePowerRequested
...
VOID QCPWR_PurgeDevicePowerQueue(PDEVICE_EXTENSION  pDevExt)
{
   ...
   if (!IsListEmpty(&pDevExt->DevicePowerRequestQueue))
   {
      ...
      (
         QCSER_DBG_MASK_PIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> PurgeDevicePowerQueue(DPWR): not empty!\n", pDevExt->PortName)
      );
   }

   while (!IsListEmpty(&pDevExt->DevicePowerRequestQueue))
   {
      headOfList = RemoveHeadList(&pDevExt->DevicePowerRequestQueue);
      req = CONTAINING_RECORD(headOfList, QCUSB_DEVICE_POWER_REQ, List);
      ExFreePool(req);
