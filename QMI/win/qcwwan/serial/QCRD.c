...
#ifdef EVENT_TRACING
#include "QCRD.tmh"      //  this is the file that will be auto generated
#endif
...
VOID CancelReadRoutine(PDEVICE_OBJECT CalledDO, PIRP pIrp)
{
   ...
   if (IoSetCancelRoutine(pIrp, NULL) != NULL)
   {
      // cancel routine has been set again by another party, probably
      // the one sets timeout. Bail out to let other party process
      // this IRP with Cancel flag set by this cancellation process.
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_READ,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> CancelReadRoutine: CxlRtn set again, bail out 0x%p\n", pDevExt->PortName, pIrp)
      );

      #ifdef QCRD_EXPERIMENT
      QcReleaseSpinLock(&pDevExt->ReadSpinLock, levelOrHandle);
      return;
      #endif // QCRD_EXPERIMENT
   }
...
)
{
   ...
   #ifdef QCRD_EXPERIMENT
   if ((pIOBlockIrp != NULL) && (CountL1ReadQueue(pDevExt) > 0))
   #else
   if (pIOBlockIrp != NULL)
   #endif // QCRD_EXPERIMENT
   {
      ...
      #ifdef QCRD_EXPERIMENT
      // set cancel routine to NULL to prevent cancel routine from being called
      if (IoSetCancelRoutine(pIrp, NULL) == NULL)
      {
      ...
      if (pIrp->Cancel)
      {
         ...
         (
            QCSER_DBG_MASK_RIRP,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> RIC<%d> RIRP 0x%p C: in callellation!! CxlReset %d\n", pDevExt->PortName,
               cookie, pIrp, pIOBlockIrp->bCxlRtnReset)
         );
      }
      if (bCompleteCallingIrp)
      {
         #ifdef QCRD_EXPERIMENT
         if ((IoSetCancelRoutine(pIrp, NULL) == NULL) && (pIOBlockIrp->bCxlRtnReset == TRUE))
         #else
         if (IoSetCancelRoutine(pIrp, NULL) == NULL)
         #endif // QCRD_EXPERIMENT
         {
      ...
      pIOBlockIrp->bCxlRtnReset = FALSE;
   }
...
BOOLEAN StartReadIntervalTimeout(PVXD_WDM_IO_CONTROL_BLOCK pIOBlock, char cookie)
{
   ...
   if (pIrp->Cancel)
   {
      ...
      (
         QCSER_DBG_MASK_READ,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> RI: IRP in cancellation 0x%p, no action\n", pDevExt->PortName, pIrp)
      );
      return FALSE;
   }
...
)
{
   ...
   if (pIOBlock->pCallingIrp == NULL)
   {
      ...
      (
         QCSER_DBG_MASK_READ,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> ReadTimeoutDpc: Error - purged IOB 0x%p\n",
...
ULONG CountL1ReadQueue(PDEVICE_EXTENSION pDevExt)
{
   ...
   /* ReadbufferCount will be fullbuffer size 
      for wrapped around scenario */ 
   if (lCount == 0)
   {
      lCount = pDevExt->lReadBufferCount;
   }
...
)
{
   ...
   if (lCharsAvailable)
   {
         ...
         <= pDevExt->pucReadBufferGet + lCharsAvailable) // .. before end of read?
      {
         ...
         bFilledSome = TRUE;
         lCharsAvailable -= lCharsNeeded;
         pDevExt->lReadBufferCount -= lCharsNeeded;
         pucTo += lCharsNeeded;
         pDevExt->pucReadBufferGet = pDevExt->pucReadBufferStart;
      ...
      {         
         ...
         pDevExt->lReadBufferCount -= lCharsAvailable;
      }
...
)
{
   ...
   (
      ...
      ("<%s> RD buf start, (%ld,%ld)-(0x%p,%ld,0x%p)-(0x%p,0x%p)\n",
   ...
   // Buffer full, drop the data on the floor
   if (ulCount > (pDevExt->lReadBufferSize - lCountInBuffer))
   {
      QCSER_DbgPrint
      (
         QCSER_DBG_MASK_READ,
   ...
   if ((pDevExt->bReadBufferReset == TRUE) && (ulCount > 0))
   {
      ...
      pDevExt->lReadBufferCount += ulCount;
      ulXferred += ulCount;
   }
   // Case 1:
      ...
      else
      {
         ...
         if (pDevExt->pucReadBufferGet <= pDevExt->pucReadBufferPut)
         {
            ...
            (
               QCSER_DBG_MASK_READ,
               QCSER_DBG_LEVEL_CRITICAL,
               ("<%s> RD buf reset, Buffer Overrun!!!(%ld,%ld)-(0x%p,%ld,0x%p)-(0x%p,0x%p)\n",
                     pDevExt->PortName, lCountInBuffer, pDevExt->lReadBufferHigh,
                     pDevExt->pucReadBufferStart, pDevExt->lReadBufferSize, pucEnd,
                     pDevExt->pucReadBufferPut, pDevExt->pucReadBufferGet
               )
...
//    usNewUartState |= SERIAL_EV_PERR;
   ...
   (
      QCSER_DBG_MASK_READ,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> RD buf end, (%ld,%ld)-(0x%p,%ld,0x%p)-(0x%p,0x%p)\n",
            pDevExt->PortName, lCountInBuffer, pDevExt->lReadBufferHigh,
            pDevExt->pucReadBufferStart, pDevExt->lReadBufferSize, pucEnd,
            pDevExt->pucReadBufferPut, pDevExt->pucReadBufferGet
      )
   ...
   if (usNewUartState)
   {
       ...
       (
          QCSER_DBG_MASK_READ,
          QCSER_DBG_LEVEL_TRACE,
          ("<%s> RD buf called ProcessNewUartState\n", pDevExt->PortName
          )
       );
...
NTSTATUS QCRD_InitializeL2Buffers(PDEVICE_EXTENSION pDevExt)
{
   ...
   if (pDevExt->bFdoReused == TRUE)
   {
      ...
      for (i = 0; i < pDevExt->NumberOfL2Buffers; i++)
      {
         KeClearEvent(&(pDevExt->pL2ReadBuffer[i].CompletionEvt));
      }
      while (!IsListEmpty(&pDevExt->L2CompletionQueue))
      {
         ...
         (
            QCSER_DBG_MASK_READ,
            QCSER_DBG_LEVEL_FORCE,
            ("<%s> InitL2: purge L2CompletionQueue!\n", pDevExt->PortName)
         );
         RemoveHeadList(&pDevExt->L2CompletionQueue);
      }
      ...
      if (pDevExt->pL2ReadBuffer[i].Buffer == NULL)
      {
         ...
         else
         {
            ...
            else
            {
               ...
               pDevExt->pL2ReadBuffer[i].bInCompletionQueue = FALSE;
               KeInitializeEvent
               (
