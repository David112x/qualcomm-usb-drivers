...
#ifdef EVENT_TRACING
#include "QCSER.tmh"      //  this is the file that will be auto generated
#endif
...
NTSTATUS QCSER_Open(PVXD_WDM_IO_CONTROL_BLOCK pIOBlock)
{
   ...
   if (inDevState(DEVICE_STATE_DEVICE_REMOVED0))
   {
      ...
      (
         QCSER_DBG_MASK_CONTROL,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> _Open: dev in DEVICE_STATE_DEVICE_REMOVED0 state\n", pDevExt->PortName)
      );
   ...
   QCWT_SendAndWaitGPSC(pDevExt, QCWT_GPSC_START);
...
NTSTATUS QCSER_Close(PVXD_WDM_IO_CONTROL_BLOCK pIOBlock)
{
   ...
   while (TRUE)
   {
      if (InterlockedIncrement(&pDevExt->ActivityInProgress) > 1)
      {
         InterlockedDecrement(&pDevExt->ActivityInProgress);

         QCSER_DbgPrint
         (
            QCSER_DBG_MASK_CONTROL,
            QCSER_DBG_LEVEL_INFO,
            ("<%s> [SYNC] _Close: wait for PnP completion\n", pDevExt->PortName)
         );

         QCSER_Wait(pDevExt, -(2 * 1000 * 1000L));  // 0.2 second
      }
      else
      {
         break;
   ...
   if (pDevExt->AggregationEnabled == 1 && pDevExt->TimerResolutionSet == 1)
   {
	  ULONG returnVal = 0;
	  ...
	  (
		 ...
		 ("<%s> MPth: Modem port closed ExSetTimerResolution returned %d\n", pDevExt->PortName, returnVal)
	  );
	  pDevExt->TimerResolutionSet = 0;
   }
   ...
   pDevExt->bClosing = TRUE;
   StopInterruptService(pDevExt, TRUE, FALSE, 4);
   pDevExt->bClosing = FALSE;
...
NTSTATUS GetModemConfig( PDEVICE_OBJECT pDevObj, PUCHAR pBuf )
{
   ...
   { // LIMIT: CDC has no firmware problem requiring cacheing of line config
      ...
      if (NT_SUCCESS(nts))
      {
         ...
         (
            QCSER_DBG_MASK_CONTROL,
            QCSER_DBG_LEVEL_DETAIL,
            ("<%s>: GetModemConfig-A: Parity %d StopBits %d DataBits %d", pDevExt->PortName,
               pMi->ucParityType, pMi->ucStopBit, pMi->ucDataBits)
         );

         // TEST - hardcode values
         if (pMi->ucParityType != 0)
         {
            pMi->ucParityType = 0;
         }
         if (pMi->ucStopBit != 0)
      ...
      (
         QCSER_DBG_MASK_READ,
         QCSER_DBG_LEVEL_DETAIL,
         ("<%s> <--GetModemConfig-A: devType %d ST: 0x%x\n", pDevExt->PortName,
           pDevExt->ucDeviceType, nts)
      );
      return nts;
   } // if DEVICETYPE_CDC
   ...
   if (pDevExt->ucDeviceType >= DEVICETYPE_SERIAL)
   {
      if (pDevExt->bModemInfoValid == TRUE)
      {
         ...
         (
            QCSER_DBG_MASK_CONTROL,
            QCSER_DBG_LEVEL_DETAIL,
            ("<%s>: GetModemConfig-B: Parity %d StopBits %d DataBits %d", pDevExt->PortName,
               pMi->ucParityType, pMi->ucStopBit, pMi->ucDataBits)
         );
         // TEST - hardcode values
         if (pMi->ucParityType != 0)
      ...
      (
         QCSER_DBG_MASK_READ,
         QCSER_DBG_LEVEL_DETAIL,
         ("<%s> <--GetModemConfig-B: devType %d ST: 0x%x\n", pDevExt->PortName,
           pDevExt->ucDeviceType, nts)
      );
      return nts;
   }
   ...
   (
      QCSER_DBG_MASK_READ,
      QCSER_DBG_LEVEL_DETAIL,
      ("<%s> <--GetModemConfig: devType %d ST: _UNSUCCESSFUL\n", pDevExt->PortName, pDevExt->ucDeviceType)
   );
...
NTSTATUS QCSER_StopDataThreads(PDEVICE_EXTENSION pDevExt, BOOLEAN CancelWaitWake, BOOLEAN Blocking)
{
   ...
   StopInterruptService(pDevExt, Blocking, CancelWaitWake, 6);
...
NTSTATUS SerialSendEncapusulatedCmd(PDEVICE_OBJECT DeviceObject, PVOID ioBuffer, PIRP pIrp)
{
   ...
   if ((Length < sizeof(UCHAR)) || (ioBuffer == NULL))  // at least one byte
   {
      pIrp->IoStatus.Information = 0;
      nts = STATUS_INVALID_PARAMETER;
   }
   else
   {
      ...
      if (pDevExt->ucDeviceType < DEVICETYPE_INVALID)
      {
         ...
         (
            QCSER_DBG_MASK_READ,
            QCSER_DBG_LEVEL_INFO,
            ("<%s> SendEncapusulatedCmd 0x%x\n", pDevExt->PortName, (UCHAR)*puc)
         );

         bufSize = sizeof(USB_DEFAULT_PIPE_REQUEST)+Length + 64; //leave enogh room for extendability 
         ...
         nts = QCUSB_PassThrough(DeviceObject, pBuf, bufSize, &ulRetSize);
         ExFreePool(pBuf);
