...
)
{
   ...
   while (TRUE)
   {
      ...
      if (!IsListEmpty(QueueToProcess))
      {
         ...
         // take care of the tag for remove lock to make WHQL/Driver Verifier happy
...
)
{
   ...
   pDevExt->SystemWakeupLevel = PowerSystemWorking;
   ...
   if (ntStatus == STATUS_SUCCESS)
   {
      pDevExt->SystemWakeupLevel = data;
      if (pDevExt->SystemWakeupLevel < PowerSystemWorking)
      {
         pDevExt->SystemWakeupLevel = PowerSystemWorking;
      }
      if (pDevExt->SystemWakeupLevel > PowerSystemHibernate)
      {
         pDevExt->SystemWakeupLevel = PowerSystemHibernate;
      }
   }
   else
   {
