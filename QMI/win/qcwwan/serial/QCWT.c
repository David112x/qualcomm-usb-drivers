...
#ifdef EVENT_TRACING
#include "QCWT.tmh"      //  this is the file that will be auto generated
#endif
...
***/
   ...
   if (inDevState(DEVICE_STATE_DEVICE_REMOVED0))
   {
      ...
      (
         QCSER_DBG_MASK_WIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> WIRP (Cdp 0x%x/%ldB) 0x%p No DEV in DEVICE_STATE_DEVICE_REMOVED0 (0x%x)\n", pDevExt->PortName,
           ntStatus, pIrp->IoStatus.Information, pIrp, pDevExt->bmDevState)
      );
      QcIoReleaseRemoveLock(pDevExt->pRemoveLock, pIrp, 2);
      IoCompleteRequest(pIrp, IO_NO_INCREMENT);
...
VOID AbortWriteTimeout(PVXD_WDM_IO_CONTROL_BLOCK pIOBlock)
{
   ...
   {
      // TimerExpired is set to TRUE in WriteIrpCompletion() (see "already cxled")
      // if the IRP's cancel routine is being called at the same time. In such a
      // case, we release remove lock only if there's a timeout value.
      ...
      else
      {
         ...
         (
            QCSER_DBG_MASK_WIRP,
            QCSER_DBG_LEVEL_DETAIL,
            ("<%s> AbortWriteTimeout: no ReleaseRML(NO TO) IOB 0x%p\n", pDevExt->PortName, pIOBlock)
         );
      }
   }
...
)
{
   ...
   if (pIOBlock->pCallingIrp == NULL)
   {
      ...
      (
         QCSER_DBG_MASK_WIRP,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> WriteTimeoutDpc: Error - purged IOB 0x%p\n",
...
BOOLEAN QCWT_GPSCSend(PDEVICE_EXTENSION pDevExt, UCHAR Command)
{
   ...
   if ((pDevExt->hWriteThreadHandle == NULL) && (pDevExt->pWriteThread == NULL))
   {
      ...
      (
         QCSER_DBG_MASK_WRITE,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> QCWT_GPSCSend: no write thread!\n", pDevExt->PortName)
      );
      return FALSE;
   }
   ...
   switch (Command)
   {
      case QCWT_GPSC_START:
      {
         dataPtr = "$GPS_START";
         dataLen = 10;
         pEvent = &pDevExt->GPSStartEvent;
         break;
      }
   ...
   if (dataLen == 0)
   {
      ...
      (
         QCSER_DBG_MASK_WRITE,
         QCSER_DBG_LEVEL_ERROR,
         ("<%s> QCWT_GPSCSend: GPS command not supported 0x%x\n", pDevExt->PortName, Command)
      );
      return FALSE;
   }

   // allocate buffer
   pBuffer = ExAllocatePoolWithTag(NonPagedPool, (dataLen+8), (ULONG)'WSPG');
   if (pBuffer == NULL)
   {
       QCSER_DbgPrint
       (
          QCSER_DBG_MASK_WRITE,
          QCSER_DBG_LEVEL_ERROR,
          ("<%s> QCWT_GPSCSend failed to alloc buf\n", pDevExt->PortName)
       );
       return FALSE;
   }
   ...
   if( pIrp == NULL )
   {
       ...
       (
          QCSER_DBG_MASK_WRITE,
          QCSER_DBG_LEVEL_ERROR,
          ("<%s> QCWT_GPSCSend failed to alloc IRP\n", pDevExt->PortName)
       );
       ExFreePool(pBuffer);
       return FALSE;
   }
   RtlCopyMemory(pBuffer, dataPtr, dataLen);
   ...
   ntStatus = QCWT_Write(pDevExt->MyDeviceObject, pIrp);
...
VOID QCWT_SendAndWaitGPSC(PDEVICE_EXTENSION pDevExt, UCHAR Command)
{
   ...
   switch (Command)
   {
      ...
      default:
      {
         ...
         (
            QCSER_DBG_MASK_WRITE,
            QCSER_DBG_LEVEL_ERROR,
            ("<%s> QCWT_SendAndWaitGPSC: invalid command %u\n", pDevExt->PortName, Command)
         );
         return;
      }
   }
   ...
   if (!NT_SUCCESS(ntStatus))
   {
      ...
      (
         QCSER_DBG_MASK_WIRP,
         QCSER_DBG_LEVEL_DETAIL,
         ("<%s> _SendAndWaitGPSC: RML failure\n", pDevExt->PortName)
      );
      return;
   }
   QcInterlockedIncrement(2, NULL, 100);

   QCSER_DbgPrint
   (
      QCSER_DBG_MASK_WRITE,
      QCSER_DBG_LEVEL_TRACE,
      ("<%s> QCWT_SendAndWaitGPSC: Function 0x%x\n", pDevExt->PortName, pDevExt->DeviceFunction)
   );
   KeClearEvent(pEvent);
   if (TRUE == QCWT_GPSCSend(pDevExt, Command))
   {
      ...
      (
         QCSER_DBG_MASK_WRITE,
         QCSER_DBG_LEVEL_TRACE,
         ("<%s> <-- QCWT_SendAndWaitGPSC\n", pDevExt->PortName)
      );
   }

   QcIoReleaseRemoveLock(pDevExt->pRemoveLock, NULL, 2);
